...
class HRPredictor:
    ...
    def __init__(
        ...
        self.model_dir = (
            Path(model_dir) if model_dir else Path(__file__).resolve().parent
        )
...
""".strip()
    ...
    def _build_raw_diagnosis(
        ...
        if remaining_tenure < 2:
            diagnosis_parts.append(
                "예상 잔여근속기간이 짧게 산출되어 조기 이탈 가능성 점검이 필요합니다."
            )
    ...
    def predict(
        ...
        remaining_tenure = max(
            expected_total_tenure - float(raw_features["tenure_years"]), 0.0
        )
        ...
        inst_avg_quit = float(inst_stats.get("avg_quit_probability", 0.0))
        inst_avg_expected_total = float(
            inst_stats.get("avg_expected_total_tenure", 0.0)
        )
    ...
    def build_features(
        ...
        features = {
            ...
            "performance_grade": self._normalize_category(
                "performance_grade", performance_grade
            ),
            "workload_level": self._normalize_category(
                "workload_level", workload_level
            ),
    ...
    def _build_lookup_tables(self) -> None:
        ...
        if missing:
            raise ValueError(
                f"employee_feature_table.csv 필수 컬럼 누락: {sorted(missing)}"
            )

        emp["tenure_years"] = pd.to_numeric(emp["tenure_years"], errors="coerce")
        emp["quit_probability"] = pd.to_numeric(
            emp["quit_probability"], errors="coerce"
        )
        emp["expected_total_tenure"] = pd.to_numeric(
            emp["expected_total_tenure"], errors="coerce"
        )
        ...
        inst_agg = emp.groupby("institution", as_index=False).agg(
            region=("region", "first"),
        ...
        # 기관 평균 통계 lookup
        inst_stats = emp.groupby("institution", as_index=False).agg(
        ...
        self.institution_stat_lookup = {
            row["institution"]: {
                ...
                "avg_expected_remaining_tenure": float(
                    row["avg_expected_remaining_tenure"]
                ),
            }
        ...
        if self.master_table is not None:
            ...
            if {"institution", "region", "institution_size", "avg_salary"}.issubset(
                master.columns
            ):
                master = master[
                    ["institution", "region", "institution_size", "avg_salary"]
                ].copy()
        ...
        self.global_numeric_defaults = {
            "institution_size": float(
                pd.to_numeric(emp["institution_size"], errors="coerce").median()
            ),
            "avg_salary": float(
                pd.to_numeric(emp["avg_salary"], errors="coerce").median()
            ),
            "regional_salary": float(
                pd.to_numeric(emp["regional_salary"], errors="coerce").median()
            ),
            "age": float(pd.to_numeric(emp.get("age"), errors="coerce").median()),
            "tenure_years": float(
                pd.to_numeric(emp.get("tenure_years"), errors="coerce").median()
            ),
        }
    ...
    def _prepare_input_dataframe(self, features: Dict[str, Any]) -> pd.DataFrame:
        ...
        for col in self.NUMERIC_COLS:
            X[col] = pd.to_numeric(X[col], errors="coerce").fillna(
                self.global_numeric_defaults[col]
            )
    ...
    def _normalize_category(self, col: str, value: Any) -> str:
        ...
        if col == "gender":
            ...
            normalized = mapping.get(
                value_str, mapping.get(value_str.lower(), value_str)
            )
    ...
    def _calculate_burnout_probability(
        ...
        # 6) 기관 환경 보정
        inst_avg_quit = self.institution_stat_lookup.get(institution, {}).get(
            "avg_quit_probability", 0.0
        )
    ...
    def _build_recommendation_summary(
        ...
        if quit_probability >= 0.70:
            messages.append(
                "퇴사위험이 높은 수준이므로 면담 우선순위를 높여 원인 점검이 필요합니다."
            )
        elif quit_probability >= 0.42:
            messages.append(
                "퇴사위험 관리 대상으로 정기 모니터링을 수행하는 것이 좋습니다."
            )

        if burnout_probability >= 0.70:
            messages.append(
                "번아웃 위험이 높아 업무부하 조정과 휴식 보장이 우선적으로 필요합니다."
            )
        elif burnout_probability >= 0.40:
            messages.append(
                "번아웃 완화를 위해 업무 분산과 역할 재조정을 검토할 필요가 있습니다."
            )

        if flexible_work == "N":
            messages.append(
                "가능하다면 유연근무 활용 여부를 함께 검토하는 것이 바람직합니다."
            )

        if workload_level == "high":
            messages.append(
                "현재 업무강도가 높은 편이므로 단기적인 부담 완화 방안 검토가 필요합니다."
            )

        if not messages:
            messages.append(
                "현재는 즉각적인 개입보다 정기적인 상태 점검 중심으로 관리하면 됩니다."
            )
